    ...
    public class AA
     {
        ...
        public AA(double[,,] city,double Lo,int Alpha,int Beta)
         {
            ...
            beta = Beta;
            lo=Lo;
            int temp = Convert.ToInt32( Math.Sqrt(city.Length/3));
            City=new double [temp,temp,3];
            Message=new double [temp,temp,3];
                ...
                for (int j = 0; j < temp; j++)
                 {
                     if (i != j)
                     {
                         for (int k = 0; k < 3; k++)
                         {
                             Message[i, j, k] = (double)1 / (temp * temp - temp);
            ...
            lock (this)
             {
                ...
                for (int i = 0; i < closed_list.Count - 1; i++)
                {
                    for (int k = 0; k < 3; k++)
                    {
                        Message[temp_Array[i], temp_Array[i + 1], k] = Message[temp_Array[i], temp_Array[i + 1], k] + lo / ((1 - lo) * Convert.ToInt32(Get_Weight(closed_list) + 1));

                    }
                }
                for (int k = 0; k < 3; k++)
                {
                    Message[temp_Array[temp_Array.Length - 1], temp_Array[0], k] = Message[temp_Array[temp_Array.Length - 1], temp_Array[0],k] + lo / ((1 - lo) * Convert.ToInt32(Get_Weight(closed_list)));
                }
                for (int i = 0; i < closed_list.Count; i++)
                {
                    for (int j = 0; j < closed_list.Count; j++)
                    {
                        for (int k = 0; k < 3; k++)
                        {
                            Message[i, j, k] = (1 - lo) * Message[i, j, k];
                        }
                    }
                }
            }
            ...
            lock (this)
             {
                ...
                for (int i = 0; i < Convert.ToInt32(temp_Array.Length) - 1; i++)
                {
                    for (int k = 0; k < 3; k++)
                    {
                        sum = sum + City[temp_Array[i], temp_Array[i + 1], k];

                    }
                }
                for (int k = 0; k < 3; k++)
                {
                    sum = sum + City[temp_Array[temp_Array.Length - 1], temp_Array[0], k];
                }
                return sum;
            }
        }
        ...
        private void NextCity()
         {
            openList.Clear();
            int temp_int=Convert.ToInt32(Math.Sqrt(City.Length/3));
        ...
        private int choiceRoute()
         {
            ...
            for (int i = 0; i < openList.Count; i++)
            {
                for (int k = 0; k < 3; k++)
                {
                    double eta = 1 / City[Pro_time, temp_Array[i], k];
                    sum_Message = sum_Message + Math.Pow(Message[Pro_time, temp_Array[i], k], alpha) * Math.Pow(eta, beta);
                }
            }
            double temp=0;
            for (int j = 0; j < openList.Count; j++)
             {
                 for (int k = 0; k < 3; k++)
                 {
                     double eta = 1 / City[Pro_time, temp_Array[j], k];
                     temp = temp + Math.Pow(Message[Pro_time, temp_Array[j], k], alpha) * Math.Pow(eta, beta) / sum_Message;
            ...
            for (int i = 0; i < 20; i++)
             {
                ...
                for (int j = 0; j < Convert.ToInt32(Math.Sqrt(City.Length/3)); j++)
                 {
                    openList.Enqueue(0);
                    closedList.Clear();
                    while (openList.Count != 0 && closedList.Count != Convert.ToInt32(Math.Sqrt(City.Length/3)))
                     {
                        int temp = openList.Dequeue();
                        Pro_time = temp;
                        closedList.Enqueue(temp);
                        if (openList.Count == 0 && closedList.Count == Convert.ToInt32(Math.Sqrt(City.Length/3)))
                         {
                            if (BestList.Count == 0)
                             {
                                int[] temp_Array = new int[Convert.ToInt32(Math.Sqrt(City.Length/3))];
                                temp_Array = closedList.ToArray();
                                for (int k = 0; k < Convert.ToInt32(Math.Sqrt(City.Length/3)); k++)
                                 {
